...
public class Peli : PhysicsGame
{
    ...
    PhysicsObject maa;
    double voima;
    [Save] AssaultRifle kk;

    [Save] DoubleMeter hpMittari;
    [Save] IntMeter miesLaskuri;
    [Save] IntMeter kopteriLaskuri;

    [Save] List<PhysicsObject> kopteriLista = new List<PhysicsObject>();
    [Save] List<PhysicsObject> tyyppiLista = new List<PhysicsObject>();
    ...
    void LuoTehtaat()
    {
        AddFactory<PhysicsObject>( "kopteri", LuoKopteri );
        AddFactory<PhysicsObject>( "tyyppi", LuoLaskuvarjo );
        AddFactory<PlatformCharacter>( "laskeutunut", LuoLaskeutunut );
    }

    void LataaKuvat()
    {
        kopterinKuvatVasemmalle = Image.Mirror( kopterinKuvat );
    ...
    PhysicsObject LuoLaskuvarjo()
    {
        return LuoLaskuvarjo( 0, 0 );
    }
    ...
    PlatformCharacter LuoLaskeutunut()
    {
        return LuoLaskeutunut(0, 0);
    }
    ...
    {
        Vector v = new Vector(tankki.X - ampuja.X, suunta * (tankki.X - ampuja.X) / RandomGen.NextDouble(2, 5));
        return v.Angle;
    }
    ...
    void AsetaNapit()
    {
        ...
        Keyboard.Listen(Key.Left, ButtonState.Down, aja, "Liiku vasemmalle", tankki, 1.0);
        Keyboard.Listen(Key.Right, ButtonState.Down, aja, "Liiku oikealle", tankki, -1.0);
        ...
        //Keyboard.Listen( Key.F5, ButtonState.Pressed, SaveGame, "Tallenna peli", "marswagen.xml" );
        //Keyboard.Listen( Key.F9, ButtonState.Pressed, LoadGame, "Tallenna peli", "marswagen.xml" );
    }
