...
public class PlatformCharacter : PhysicsObject
{
    ...
    public bool IsAboutToFall()
    {
       ...
       */
        ...
        {
            if (collisionHelpers[i].Object.Bottom < lowestPoint)
                lowestPoint = collisionHelpers[i].Object.Bottom;
        }
        ...
        if (platform is not PhysicsObject p)
            return true;
        if (p.IgnoresCollisionResponse)
            return true;
        if (p.CollisionIgnoreGroup == this.CollisionIgnoreGroup && p.CollisionIgnoreGroup != 0)
            return true;
    ...
    {
        if (target == null || target.IsDestroyed || this.IgnoresCollisionWith(target))
            return false;
