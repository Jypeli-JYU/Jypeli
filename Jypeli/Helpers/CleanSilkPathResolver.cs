...
// Obtained from https://github.com/dotnet/Silk.NET/blob/0dc3175e7b3c037c645022030150af022f8be8d9/src/Core/Silk.NET.Core/Loader/DefaultPathResolver.cs
// 
// FIXME: Remove when Linux resolving is fixed in mainstream
...
namespace Jypeli.Helpers
{
    ...
    /// </summary>
    /// <remarks>
    /// Note (dezhidki): The default path resolver does not acutually work like this in original Silk.NET (v. 2.2.0).
    /// The default version prioritizes the library without name (i.e. the OS default resolve order) and only then those relative to the application.
    /// This "clean" version only tries resolve libraries relative to the final app. Since Jypeli uses GLFW,
    /// this allows running the app using the GLFW builds included with Silk.NET.
    /// </remarks>
    internal class CleanSilkPathResolver : PathResolver
    {
        /// <summary>
        /// Creates a path resolver pre-loaded with the default <see cref="Resolvers"/>.
        /// </summary>
        public CleanSilkPathResolver() => Resolvers = new()
        {
            BaseDirectoryResolver,  // resolve all to base directory here => this forces the first path be relative to app
            LinuxVersioningResolver,  // Add linux version variations
        ...
        public static readonly Func<string, IEnumerable<string>> BaseDirectoryResolver = name
            ...
            => string.IsNullOrEmpty(AppContext.BaseDirectory) && string.IsNullOrWhiteSpace(Path.GetDirectoryName(name))
                ? Enumerable.Repeat(Path.Combine(AppContext.BaseDirectory, Path.GetFileName(name)), 1)
        ...
        public static readonly Func<string, IEnumerable<string>> SilkDirectoryResolver = name =>
        {
            try
            {
                var asmLocation = typeof(CleanSilkPathResolver).Assembly.Location;
                // check that name doesn't have a directory name, we only want raw filenames so that the Path.Combine
                // doesn't blow up.
                if (!string.IsNullOrWhiteSpace(Path.GetDirectoryName(name)) &&
                    !string.IsNullOrWhiteSpace(asmLocation) && File.Exists(asmLocation))
                {
                ...
                asmLocation = typeof(CleanSilkPathResolver).Assembly.CodeBase;
                if (!string.IsNullOrWhiteSpace(Path.GetDirectoryName(name)) &&
                    !string.IsNullOrWhiteSpace(asmLocation) && File.Exists(asmLocation))
                {
                    asmLocation = Path.GetDirectoryName(asmLocation);
        ...
        )
        {
            try
            {
                ...
                var currentRid = GetNormalizedRuntimeIdentifier();
                ...
                foreach (var nativeAsset in runtimeLib.GetRuntimeNativeAssets(defaultContext, rid))
                {
                    ...
                    if (Path.GetFileName(nativeAssetSpan) == name ||
                        Path.GetFileNameWithoutExtension(nativeAssetSpan) == name)
#endif
                    {
        ...
        private static bool TryLocateNativeAssetInRuntimesFolder(string name, string baseFolder, out string? result)
        {
            ...
            foreach (var rid in GetAllRuntimeIds(GetNormalizedRuntimeIdentifier(), DependencyContext.Default))
            {
        ...
        private static bool AddFallbacks(List<string> fallbacks, string? rid,
            IReadOnlyList<RuntimeFallbacks> allFallbacks)
        {
        ...
        {
            // Current native libs are not specific to specific Linux distro;
            // as such, for Jypeli purposes it's easier to just hadle Linux more generally
            // than trying to parse its distro name
            var os = RuntimeEnvironment.OperatingSystemPlatform switch
            {
            ...
            return $"{os}-{RuntimeEnvironment.RuntimeArchitecture}";
        }
    }
}