...
public class GenerationFunctions
{
    ...
    public string DetectWindows10InstalledSDK()
    {
		...
		var path = System.IO.Path.Combine(ProgramFilesx86(), @"Windows Kits\10\bin\");
		if (!System.IO.Directory.Exists(path + productVersion))
		{
			productVersion += ".0";
			if (!System.IO.Directory.Exists(path + productVersion))
			{
				Console.Error.WriteLine("WARNING: Could not find the SDK directory");
			}
		}
